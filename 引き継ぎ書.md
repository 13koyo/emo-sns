...
### 15. お守り機能（Phase 2 MVP）実装
- **機能**:
  - 各投稿に「お守りボタン（🔖）」を追加
  - `/amulet` ページで保存した投稿を一覧表示
  - ローカルストレージを使用したブラウザ保存（ログイン不要）
- **ファイル**: `src/hooks/useAmulet.ts`, `src/components/AmuletButton.tsx`, `src/app/amulet/page.tsx`
- **ステータス**: 実装・ビルド確認完了
...
### 優先（Phase 1 残り / 修正）
- [ ] 🚀 **お守り機能の動作確認（デプロイ）** ← 次回のタスク（GitHubプッシュ & Vercel確認）
- [ ] 🐛 GA4のデータ計測がされない問題を修正（反映待ち）
- [ ] ユーザー認証（匿名 + 任意アカウント）

### Phase 2: AI導入 & ユーザー拡大
- [x] お守り機能（パーソナル・ギャラリー）— MVPコード実装済み（デプロイ待ち）
